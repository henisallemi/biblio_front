...
class _ParametreState extends State<Parametre> {
  ...
  Future<void> updatePassword() async {
    if (confirmerMotDePasse.text != nouveauMotDePasse.text) {
      showDialog(
        context: context,
    ...
    if (nouveauMotDePasse.text.length < 8 ||
        !RegExp(r'[A-Z]').hasMatch(nouveauMotDePasse.text) ||
        !RegExp(r'[a-z]').hasMatch(nouveauMotDePasse.text) ||
        !RegExp(r'[0-9]').hasMatch(nouveauMotDePasse.text) ||
        !RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(nouveauMotDePasse.text)) {
      String errorMessage =
          'Le mot de passe doit respecter les critères suivants :\n\n';
      if (nouveauMotDePasse.text.length < 8) {
        errorMessage += '- Au moins 8 caractères\n';
      }
      if (!RegExp(r'[A-Z]').hasMatch(nouveauMotDePasse.text)) {
        errorMessage += '- Au moins une lettre majuscule\n';
      }
      if (!RegExp(r'[a-z]').hasMatch(nouveauMotDePasse.text)) {
        errorMessage += '- Au moins une lettre minuscule\n';
      }
      if (!RegExp(r'[0-9]').hasMatch(nouveauMotDePasse.text)) {
        errorMessage += '- Au moins un chiffre\n';
      }
      if (!RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(nouveauMotDePasse.text)) {
        errorMessage +=
    ...
    var body = json.encode({
      "oldPassword": ecrireMotDePasse.text,
      "newPassword": nouveauMotDePasse.text
    });
    var uri = Uri.parse(
        'http://localhost:4000/api/users/${userState.connectedUser!.id}/password');
